...
class RecommendationEngine:
    def __init__(self, mongo_uri="mongodb://localhost:27017/", db_name="movie_recommender"):
        ...
        self.user_ratings_collection = self.db["user_ratings"]  # Use the same collection
        self.preferences_collection = self.db["user_preferences"] # and the same collection
        self.new_df = None
        self.movies = None
        self.movies2 = None
    ...
    #     # Remove the file loading logic, not necessary with MongoDB
    #     raise NotImplementedError("File loading is not used with MongoDB")
    ...
    def get_collaborative_recommendations(self, username, n=10):
        ...
        # Load all user ratings from MongoDB
        all_ratings = {}
        for rating_doc in self.user_ratings_collection.find():
            user = rating_doc['username']
            if 'ratings' in rating_doc:
                for movie_id, rating in rating_doc['ratings'].items():
                    if user not in all_ratings:
                        all_ratings[user] = {}
                    all_ratings[user][str(movie_id)] = rating
        ...
        for user, movie_ratings in all_ratings.items():
            for movie_id, rating in movie_ratings.items():
                ratings_data.append({
                    'user': user,
                    'movie_id': int(movie_id),  # Convert back to integer
                    'rating': rating
                })
